...
class ClaudeAwsClient(LLMApiClient):
    ...
    def _stream_response(self, message: str, max_tokens: int) -> Generator[str, None, None]:
    ...
    def _stream_one_response(self, msg: List[Dict[str, str]], max_tokens: int) -> Generator[str, None, None]:
        response = self.client.messages.create(
            model=self.model,
            max_tokens=max_tokens,
            messages=msg,
...
class ContinuousStreamIterator:
    ...
    def _finalize_initial_response(self):
        ...
        if self.tool_uses:
            ...
            logger.debug("No tool uses found, transitioning to final_response state")
    ...
    def _handle_tool_calls(self):
        if self.tool_uses:
            ...
            logger.debug("No more tool calls, transitioning to final_response state")
            self.state = 'final_response'
    ...
    def _handle_final_response(self):
        ...
        if self.is_stream:
            try:
                ...
                logger.debug("Final response complete, transitioning to finished state")
                self.state = 'finished'
        else:
    ...
    def _process_event(self, event):
        ...
        if event.type == 'content_block_start':
            ...
            if hasattr(event.delta, 'tool_calls'):
                for tool_call in event.delta.tool_calls:
                    logger.debug(f"Adding tool call from message_delta: {tool_call.name}")
                    self.tool_uses.append(tool_call)
