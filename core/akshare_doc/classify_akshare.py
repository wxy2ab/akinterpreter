...
def classify_akshare_functions(akshare_all_functions: str, llm_client: LLMApiClient) -> Dict[str, List[str]]:
    # 定义主要分类
    categories = [
    ...
    # VSCode 兼容的正则表达式
    pattern = r'\s*"([^"]+)"\s*,?\s*#\s*(.*?)(?=\n\s*"|$)'
    functions = re.findall(pattern, akshare_all_functions, re.DOTALL | re.MULTILINE)
...
def save_classified_functions(classified_functions: Dict[str, List[str]], file_path: str):
    with open(file_path, 'w', encoding='utf-8') as f:
        json.dump(classified_functions, f, ensure_ascii=False, indent=2)

# 打印分类结果
def print_classified_functions(classified_functions: Dict[str, List[str]]):
    total_functions = 0
...
# 使用示例
# claude_client = ClaudeAwsClient()
# classified_functions = classify_akshare_functions(akshare_all_functions, claude_client)
# save_classified_functions(classified_functions, "classified_akshare_functions.json")
# print_classified_functions(classified_functions)